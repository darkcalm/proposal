...
% Digital twin arrows
\draw[arrow] (DT) -- node[edgelabel, right, yshift=6pt] {Operates Within\\(MCP/A2A/ANP)} (EC);
\draw[arrow] (EC) -- node[edgelabel, left, yshift=-6pt] {Drives Simulation} (DT);

% Core relationships (horizontal)
\draw[arrow] (HEW) -- node[edgelabel, above] {Provides Basis For\\(MCP/A2A/ANP Protocols)} (DT);
